...
baseline_throughput = 100000 * ratios[target] / (sum(times)/len(times)) 
...
#             if line_output.startswith(time_prefix):
#                 times.append(float(line_output[len(time_prefix):]))
#     throughput = 100000 * ratios[target] / (sum(times)/len(times))
...
with open(f"results/groundtruth-{idx}.log", 'r') as f:
    for line in f.readlines():
        ...
        if line_output.startswith(time_prefix):
            times.append(float(line_output[len(time_prefix):]))
groundtruth = 100000 / (sum(times)/len(times))
...
with open(f"results/slowdown-{idx}.log", 'r') as f:
    for line in f.readlines():
        ...
        if line_output.startswith(time_prefix):
            times.append(float(line_output[len(time_prefix):]))
throughput = 100000 / (sum(times)/len(times))
