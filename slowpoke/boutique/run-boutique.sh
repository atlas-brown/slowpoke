...
num_exp=10
DIR=boutique/04-06-all-flush-time-based-sleep
FILE=mix-$target-t$thread-c$conn-r$repetitions-req$num_req-n$num_exp-batch$poker_batch-all-flush-relative100reqbatch.log
mkdir -p $DIR
if [ -f $DIR/$FILE ]; then
